...
Page({
  ...
  onLoad: function (options) {
    this.setData({ teacherId: Number(options.teacherId), timeFlag: options.timeFlag ? options.timeFlag : 'day'})
    let that = this;
    wx.getStorage({
      key: 'userInfo',
      success(res) {
  ...
  getTeacherData(){
    ...
    Http.get('/employee/return/visit/detatiled', {
      ...
      if (res.result == 1000) {
        ...
        dataDetail.list = dataDetail.exceptClueList;
        this.setData({
          dataDetail
        })
      } 
  ...
  yhflist(){
    var dataDetail = this.data.dataDetail;
    dataDetail.list = dataDetail.alreadyClueList;
    this.setData({
      dataDetail
    })
  }
